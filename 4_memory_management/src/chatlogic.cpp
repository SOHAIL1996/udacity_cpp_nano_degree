...
ChatLogic::ChatLogic()
{
    ...
    // create instance of chatbot ----------------------------- Moves via ptr now 
...
ChatLogic::~ChatLogic()
{
    ...
    // delete chatbot instance ------------------ No longer used as it is being moved via ptr
    // delete _chatBot; 

    // delete all nodes ---------------- Redundant due to use of unique ptr
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // check if node with this ID exists already ----------------------- Adapting for unique ptr
                        ...
                        {
                            // _nodes.emplace_back(new GraphNode(id)); --------------- Adapt asssignment to unique ptr
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // get iterator on incoming and outgoing node via ID search ----- Adapt to work with unique ptr
                            ...
                            // create new edge --- Adapt for unqiue ptr
                            // GraphEdge *edge = new GraphEdge(id);
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);

                            edge->SetChildNode((*childNode).get());
                            edge->SetParentNode((*parentNode).get());
                            // _edges.push_back(edge); -- Move instead
                            ...
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // Move edge instead of vector passing
                        }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root ---- Adapt for unique ptr
            }
            else
            {
